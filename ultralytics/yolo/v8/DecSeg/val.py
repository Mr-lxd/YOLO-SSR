            ...
            if self.args.verbose and not self.training and self.nc > 1 and len(self.stats[label_name]):
                for i, c in enumerate(self.metrics[count].ap_class_index):
                    if self.data['map'][count]!='None':
                        ...
                        #####LXD
                        ##### modified to handle detection tasks separately, avoiding KeyError in self.seg_result for non-segmentation tasks.
                        if 'seg' in label_name:
                            key_values = [(key, value.avg) for key, value in
                                          self.seg_result[label_name].items()]
                            LOGGER.info(pf % (self.names[class_index], *sum(key_values, ())))
                        else:
                            LOGGER.info(pf % (
                            self.names[class_index], self.seen[label_name], self.nt_per_class[label_name][c],
